...
<script>
import { GetMenu, findMenusByRoleId } from '@/api/menu'
import { rolesMenus } from '@/api/role'
  ...
  methods: {
    insert: function () {
      ...
      this.$refs.form.validate((valid, error) => {
        if (valid) {
          ...
          this.loading = true
          rolesMenus(submitData).then(({ data }) => {
            if (data.resp_code === 0) {
              this.loading = false
              this.$notify({
                title: '成功',
    ...
    open: function (row) {
      ...
      this.$nextTick(() => {
        ...
        findMenusByRoleId(self.form.id).then(response => {
